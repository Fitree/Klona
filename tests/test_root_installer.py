...
class RootInstallerTests(unittest.TestCase):
    ...
    def test_claude_install_routes_to_install_only(self):
        with mock.patch("klona_agent.claude.install.install") as install, mock.patch(
            "klona_agent.claude.install.uninstall"
        ) as uninstall:
            result = install_agent.main(["--platform", "claude"])

        self.assertEqual(result, 0)
        install.assert_called_once_with(mcp_url=None, mcp_token=None)
        uninstall.assert_not_called()

    def test_claude_install_passes_mcp_args_to_install(self):
        with mock.patch("klona_agent.claude.install.install") as install, mock.patch(
            "klona_agent.claude.install.uninstall"
        ) as uninstall:
            result = install_agent.main(
    ...
    def test_claude_uninstall_routes_to_uninstall_only(self):
        with mock.patch("klona_agent.claude.install.install") as install, mock.patch(
            "klona_agent.claude.install.uninstall"
        ) as uninstall:
            result = install_agent.main(["--platform", "claude", "--uninstall"])

        self.assertEqual(result, 0)
        uninstall.assert_called_once_with()
        install.assert_not_called()

    def test_claude_uninstall_with_mcp_args_routes_to_uninstall_only(self):
        with mock.patch("klona_agent.claude.install.install") as install, mock.patch(
            "klona_agent.claude.install.uninstall"
        ) as uninstall:
            result = install_agent.main(
